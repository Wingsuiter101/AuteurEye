...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDirectors = async () => {
      ...
      try {
        ...
        if (isMounted) {
          // Preprocess: ensure every director has a valid profile_path
          const processedDirectors = detailedDirectors.map(d => ({
            ...d,
            profile_path: d.profile_path || (d.gender === 1 ? placeholderFemale : d.gender === 2 ? placeholderMale : placeholderAmbiguous)
          }));
          setDirectors(processedDirectors);
          // Cache the new data
          localStorage.setItem(CACHE_KEY, JSON.stringify({
            data: processedDirectors,
            timestamp: Date.now()
          }));
        }
      } catch (err) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-auteur-bg" role="status" aria-label="Loading">
        <LoadingCamera />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-auteur-bg p-4" role="alert">
        <div className="max-w-md w-full bg-white/5 backdrop-blur-lg rounded-xl p-8 text-center border border-white/10">
          <p className="text-red-500 mb-4">{error}</p>
          <button
            onClick={() => setError(null)}
            className="px-6 py-3 bg-auteur-accent hover:bg-auteur-accent-dark text-white rounded-lg font-medium transition-colors duration-300"
          >
            Try Again
          </button>
        </div>
                      ...
                      >
                        <img
                          ...
                          onError={e => {
                            ...
                            target.onerror = null;
                            target.src = director.gender === 1 ? placeholderFemale : director.gender === 2 ? placeholderMale : placeholderAmbiguous;
                          }}
                        />
                      </motion.div>
                            ...
                            {director.directed_movies.slice(0, 2).map(movie => (
                              ...
                              >
                                <div className="w-10 h-14 rounded-md bg-auteur-neutral/20 flex-shrink-0 border border-white/10 overflow-hidden shadow-md"> 
                                  {movie.poster_path ? (
                                    <img src={movie.poster_path ? getImageUrl(movie.poster_path) : director.gender === 1 ? placeholderFemale : director.gender === 2 ? placeholderMale : placeholderAmbiguous} alt={movie.title} className="w-full h-full object-cover"/>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full overflow-hidden bg-auteur-neutral/20 flex-shrink-0 ring-1 ring-white/10"> 
                        <img
                          src={director.profile_path ? (director.profile_path.startsWith('/') ? getImageUrl(director.profile_path) : director.profile_path) : (director.gender === 1 ? placeholderFemale : director.gender === 2 ? placeholderMale : placeholderAmbiguous)}
                          alt={director.name}
                          className="w-full h-full object-cover"
                          onError={e => {
                            const target = e.currentTarget;
                  ...
                  >
                    ...
                    <div className="relative h-auto min-h-[24rem] md:h-96 rounded-xl overflow-hidden shadow-lg"> 
                      <div 
                        ...
                        style={{ 
                          backgroundImage: `url(${activeDirector.profile_path ? getImageUrl(activeDirector.profile_path) : activeDirector.gender === 1 ? placeholderFemale : activeDirector.gender === 2 ? placeholderMale : placeholderAmbiguous})`,
                        ...
                        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-10 w-full">
                          ...
                          >
                            {activeDirector.profile_path ? (
                              <img
                                src={activeDirector.profile_path ? (activeDirector.profile_path.startsWith('/') ? getImageUrl(activeDirector.profile_path) : activeDirector.profile_path) : (activeDirector.gender === 1 ? placeholderFemale : activeDirector.gender === 2 ? placeholderMale : placeholderAmbiguous)}
                                alt={activeDirector.name}
                                className="w-full h-full object-cover"
                                onError={e => {
                                  const target = e.currentTarget;
                                  target.onerror = null;
                                  target.src = activeDirector.gender === 1 ? placeholderFemale : activeDirector.gender === 2 ? placeholderMale : placeholderAmbiguous;
                                }}
                              />
                            ) : (
                          ...
                          {activeDirector.directed_movies.slice(0, 3).map(movie => (
                            ...
                            >
                              <div className="w-12 h-16 rounded-md overflow-hidden bg-auteur-neutral/20 flex-shrink-0 border border-white/10"> 
                                {movie.poster_path ? (
                                  <img
                                    src={movie.poster_path ? getImageUrl(movie.poster_path) : activeDirector.gender === 1 ? placeholderFemale : activeDirector.gender === 2 ? placeholderMale : placeholderAmbiguous}
                                    alt={movie.title}
                                    className="w-full h-full object-cover"
                                  />
                                ) : (
